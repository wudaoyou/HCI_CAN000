...
class Domain {
	...
	public void parse(def xml) {
		XMLCreationHelper helper = new XMLCreationHelper();
		...
		districts.each {
			collectData(FODistricts, (String)it.externalCode, (String)it.effectiveStartDate, (String)it.effectiveEndDate, (String)it.externalName_defaultValue);
		}

		def regions = xml.'multimap:Message1'.cust_Region.cust_Region;
		regions.each {
			collectData(FORegions, (String)it.externalCode, (String)it.effectiveStartDate, (String)it.effectiveEndDate, (String)it.externalName_defaultValue);
		}
		...
		picklistV2.each {
			...
			option.non_uniqueCode = it.nonUniqueExternalCode;
		...
		locations.each {
			...
			loc.description = helper.getCurrent(it.name);
			loc.type = helper.getCurrent(it.customString3Nav.cust_locationType.locationTypeCode);
			loc.typeDesc = helper.getCurrent(it.customString3Nav.cust_locationType.locationTypeName);
			loc.subType = helper.getCurrent(it.customString4Nav.PicklistOption.externalCode);	
			def labels = it.customString4Nav.PicklistOption.picklistLabels.PicklistLabel;
			for (lbl in labels) {
				if (lbl.locale == 'en_US') {
					loc.subTypeDesc = lbl.label;
					break;
				}
			}
			loc.addressLine1 = helper.getCurrent(it.addressNavDEFLT.FOCorporateAddressDEFLT.address1);
			loc.city = helper.getCurrent(it.addressNavDEFLT.FOCorporateAddressDEFLT.city);
			loc.zipcode = helper.getCurrent(it.addressNavDEFLT.FOCorporateAddressDEFLT.zipCode);
			loc.country = helper.getCurrent(it.addressNavDEFLT.FOCorporateAddressDEFLT.country);
			loc.province = helper.getCurrent(it.addressNavDEFLT.FOCorporateAddressDEFLT.province);
			loc.addressStartDate = helper.getCurrent(it.addressNavDEFLT.FOCorporateAddressDEFLT.startDate);
			loc.addressStartDate = loc.addressStartDate.substring(0,10);
			loc.addressEndDate = helper.getCurrent(it.addressNavDEFLT.FOCorporateAddressDEFLT.endDate)
			loc.addressEndDate = loc.addressEndDate.substring(0,10);
			FOObject country_data = findPicklistValue("csfCountry", loc.country, '2000-01-01');
			loc.country = country_data == null ?"":country_data.description;
		...
		tz.daylight_Code = null;
		tz.daylight_Description = null;
...
class Picklist extends DataObject {
	...
	void fillPicklistV2(Domain d, def code, String picklistId, String startDate) {
		a003_Code = code; //(code == "" ? null : code);
		if (a003_Code == null || a003_Code == "") return;
	...
	void fillPicklistV2_nonUnique(Domain d, def code, String picklistId, String startDate) {
		String temp = code; //(code == "" ? null : code);
		if (temp == null || temp == "") return;
		FOListOption op = d.findPicklistValue(picklistId, temp, startDate);
		if (op != null) {
			a006_Description = op.description;
			a003_Code = op.non_uniqueCode;
		}
	}
...
class Event extends DataObject {
	...
	String a003_StartDate;
	//String a006_EndDate;
	String a006_EstimatedReturnDate;
...
class XMLCreationHelper {
	...
	public String translateYesNo(String value) {
		...
		while (toProcessNodeStack.size() > 0) {
			...
			if (current.data == null) {
				...
				if (current.data.equals("")) {					
					def attr = ["xsi:nil":"true"];
					if (current.nodeName.contains("Date") || current.nodeName.contains("Amount") ||
						current.nodeName.contains("WorkHours") || 
						current.nodeName.contains("Indicator") )
						tag = marker.createNode(current.nodeName, attr);
					else
						tag = marker.createNode(current.nodeName);
					marker.nodeCompleted(null, tag);
				} else {
...
def Message processData(Message message) {
	...
	String timestamp = map.get("MSG_TIMESTAMP");
	String changedSegmentOnly = map.get("CHANGED_SEGMENT_ONLY");
	String eeProcessed = "";
	...
	compoundEEs.each { //compoundEEs
		...
		String prevBirthDate = helper.getPrevious(it.person.date_of_birth);
		...
		for (def a in it.person.address_information) {
			...
			addr.a006_EndDate =  a.end_date.size()==0?"9999-12-31":a.end_date;
			//if (addr.a006_EndDate < FTSD) continue;
			prev_addr.a009_Purpose = new Picklist();
			prev_addr.a009_Purpose.checkObject();
			prev_addr.a009_Purpose.fillPicklistV2(domain, helper.getPrevious(a.address_type), "addressType", addr.a003_StartDate);
			prev_addr.a009_Purpose.checkObject()
			prev_addr.a012_Line1 = helper.getPrevious(a.address1);
			prev_addr.a015_Line2 = helper.getPrevious(a.address2);
			prev_addr.a018_Line3 = helper.getPrevious(a.address3);
			prev_addr.a021_Line4 = helper.getPrevious(a.address4);
			prev_addr.a024_Line5 = helper.getPrevious(a.address5);
			prev_addr.a027_Line6 = helper.getPrevious(a.address6);
			prev_addr.a030_City = helper.getPrevious(a.city);
			prev_addr.a039_PostalCode = helper.getPrevious(a.zip_code);
			prev_addr.a042_Country = new Country();
			prev_addr.a042_Country.a012_ISO3CharacterCode = helper.getPrevious(a.country);
			...
			prev_addr.a036_MajorSubdivision.a003_AbbreviatedName = helper.getPrevious(a.state);
			...
			addr.a036_MajorSubdivision.a003_AbbreviatedName = helper.getCurrent(a.state);
			...
			addr.checkObject();
			if (changedSegmentOnly.equals("TRUE")) {
			    if (prev_addr.is_empty && addr.a000_Action == "CHANGE")
                    addr.is_empty = true;                
			} 
		...
		for (def ph in it.person.phone_information) {
			...
			prev_phone.a009_Purpose.fillPicklistV2(domain, helper.getPrevious(ph.phone_type), "ecPhoneType", "2001-01-01");
			prev_phone.a009_Purpose.checkObject();
			rawNumber = helper.getPrevious(ph.phone_number);
			if (rawNumber != null) {
			...
			if (p.a001_SourceDateTime) {
			 p.a003_StartDate = p.a001_SourceDateTime.substring(0, 10);
			}
		...
		for (def e in it.person.email_information) {
			...
			prev_email.a009_Purpose.fillPicklistV2(domain, helper.getPrevious(e.email_type), "ecEmailType", "2001-01-01")
			prev_email.a009_Purpose.checkObject();
			prev_email.a012_Address = helper.getPrevious(e.email_address);
			prev_email.a015_PrimaryIndicator = helper.getPrevious(e.isPrimary);
			...
			if (email.a001_SourceDateTime != null)
				email.a003_StartDate = email.a001_SourceDateTime.substring(0, 10);
			email.checkObject();
            if (changedSegmentOnly.equals("TRUE")) {
		...
		for (def p in it.person.personal_information) {
			...
			pOut.a004_EndDate = p.end_date.size()==0?"9999-12-31":p.end_date;
			...
			if (pOut.a004_EndDate == '9999-12-31') {
				...
				//domain_data = domain.findPicklistValue("defaultLocale", curr_lang.a006_ISO2CharacterLanguageCode, '2016-01-01');
				//curr_lang.a003_Name = (domain_data == null) ? null : domain_data.description;
				curr_lang.a003_Name = helper.getCurrent(p.custom_string2);
			...
			pOut.a006_Name.a020_Prefix = helper.getCurrent(p.salutation);
			pOut.a006_Name.checkObject();
			pOut.a007_PreferredName.a003_FirstName = helper.getCurrent(p.display_name);
			...
			if (pOut.a004_EndDate == '9999-12-31') {
				...
				prev_lang.a006_ISO2CharacterLanguageCode = helper.getPrevious(p.native_preferred_lang);
				//domain_data = domain.findPicklistValue("defaultLocale", prev_lang.a006_ISO2CharacterLanguageCode, '2016-01-01');
				//prev_lang.a003_Name = (domain_data == null) ? null : domain_data.description;
				prev_lang.a003_Name = helper.getPrevious(p.custom_string2);
				prev_lang.checkObject();
				//prevPerson.a022_PreferredLanguage = prev_lang;
			}
			prevPerson.a015_BirthDate = prevBirthDate;
			prevPerson.a006_Name.a021_FullName = helper.getPrevious(p.formal_name);
			prevPerson.a006_Name.a003_FirstName = helper.getPrevious(p.first_name);
			prevPerson.a006_Name.a012_LastName = helper.getPrevious(p.last_name);
			prevPerson.a006_Name.a006_MiddleName1 = helper.getPrevious(p.middle_name);
			prevPerson.a006_Name.a015_AcademicTitle = helper.getPrevious(p.name_prefix);
			prevPerson.a006_Name.a020_Prefix = helper.getPrevious(p.salutation);
			prevPerson.a006_Name.checkObject();
			prevPerson.a007_PreferredName.a003_FirstName = helper.getPrevious(p.display_name);
			prevPerson.a007_PreferredName.a012_LastName = helper.getPrevious(p.second_last_name);
			prevPerson.a007_PreferredName.checkObject();
			prevPerson.a021_MaritalStatus.fillPicklistV2(domain, helper.getPrevious(p.marital_status), "ecMaritalStatus", pOut.a003_StartDate);
			prevPerson.a021_MaritalStatus.checkObject();
			prevPerson.a009_Gender.fillPicklistV2(domain, helper.getPrevious(p.gender), "Gender", pOut.a003_StartDate);
			prevPerson.a009_Gender.checkObject();
			prevPerson.a018_Nationality.a012_ISO3CharacterCode = helper.getPrevious(p.nationality);
			...
			if (changedSegmentOnly.equals("TRUE")) {
			    if (prevPerson.is_empty && pOut.a000_Action == "CHANGE")
			        pOut.is_empty = true;
			} 
			
			if (pOut.a000_Action == "NO CHANGE" && !prevPerson.is_empty) 
				pOut.a000_Action = "CHANGE";
					
			if (!pOut.is_empty)
				eeOut.a006_Person.push(pOut);
		} // end of person.personal_information
		
		if (eeOut.a006_Person.size() == 0 /*&& prevBirthDate != null*/) {
			Person emptyP = new Person();
			emptyP.a015_BirthDate = birthDate;			
			emptyP.a005_BusinessPartnerIdentifier = pernr;			
			Person prevEmptyP = new Person();
			prevEmptyP.a015_BirthDate = prevBirthDate;
			prevEmptyP.checkObject();
			emptyP.a024_PreviousPerson = prevEmptyP;
			if (prevBirthDate != null) {
				emptyP.a000_Action = "CHANGE";
			} 
			emptyP.checkObject();
		...
		for (def id in it.person.national_id_card) {
			...
			doc.a012_IssuingCountry = new Country();
			doc.a012_IssuingCountry.a012_ISO3CharacterCode = helper.getCurrent(id.country);
			doc.a012_IssuingCountry.checkObject();
			if (helper.getCurrent(id.country) == "CAN") {
				doc.a006_Description = "SocialInsuranceNumber";//helper.getCurrent(id.card_type);
			}
		
			prev_doc.a003_Number = helper.getPrevious(id.national_id);
			prev_doc.a012_IssuingCountry = new Country();
			prev_doc.a012_IssuingCountry.a012_ISO3CharacterCode = helper.getPrevious(id.country);
			prev_doc.a012_IssuingCountry.checkObject();
			prev_doc.checkObject();
			//if (!prev_doc.is_empty) {
			//	prev_doc.a006_Description = "SocialInsuranceNumber";				
			//}
			...
			if (changedSegmentOnly.equals("TRUE")) {
			    if (prev_doc.is_empty && doc.a000_Action == "CHANGE") 
			        doc.is_empty = true;
			} 
			...
			if (!doc.is_empty) {
				//Person emptyP = new Person();
				Person emptyP = eeOut.a006_Person.get(0);
		...
		for (def emp in it.person.employment_information) {
			...
			for (def job_event in emp.job_event_information) {
				...
				prev_event.a009_EventCategory.fillPicklistV2(domain, helper.getPrevious(job_event.event), "event", event.a003_StartDate);
				prev_event.a009_EventCategory.checkObject();
				prev_event.a012_EventReason = new Picklist();
				prev_event.a012_EventReason.fillPicklistWithDomain(domain, domain.FOEventReasons, helper.getPrevious(job_event.event_reason), event.a003_StartDate);
			...
			for (def job in emp.job_information) {
				...
				for (Event ev in eeOut.a069_Event) {
					...
					if (ev.a003_StartDate == job_start_date && ev.a009_EventCategory.a003_Code == curr_event &&
						...
						String expected_date = helper.getPrevious(job.expected_return_date);
						if (expected_date != null) {
							ev.a015_PreviousEvent.a006_EstimatedReturnDate = expected_date;
						}
						expected_date = helper.getCurrent(job.expected_return_date);
						if (expected_date != null)
							ev.a006_EstimatedReturnDate = expected_date;
						ev.a015_PreviousEvent.checkObject();
						ev.checkObject();
					}
				}
				...
				status.a012_EndDate = job.end_date.size()==0?"9999-12-31":job.end_date;
				
				if (job.end_date == '9999-12-31') {
					eeOut.a054_CSRPIndicator = helper.getCurrent(job.custom_string17);
					eeOut.a054_CSRPIndicator = helper.translateYesNo(eeOut.a054_CSRPIndicator);
					eeOut.a039_PayrollClerkIndicator = helper.getCurrent(job.custom_string19);
					eeOut.a039_PayrollClerkIndicator = helper.translateYesNo(eeOut.a039_PayrollClerkIndicator);
					
					eeOut.a078_PreviousEmployee.a054_CSRPIndicator = helper.getPrevious(job.custom_string17);
					eeOut.a078_PreviousEmployee.a054_CSRPIndicator =  helper.translateYesNo(eeOut.a078_PreviousEmployee.a054_CSRPIndicator);
					eeOut.a078_PreviousEmployee.a039_PayrollClerkIndicator = helper.getPrevious(job.custom_string19);
					eeOut.a078_PreviousEmployee.a039_PayrollClerkIndicator = helper.translateYesNo(eeOut.a078_PreviousEmployee.a039_PayrollClerkIndicator);
				}
				
				prev_status.a003_Code = helper.getPrevious(job.emplStatus);
				...
				if (changedSegmentOnly.equals("TRUE")) {
				    if (prev_status.is_empty && status.a000_Action.equals("CHANGE")) {
				    	...
				    	status.a000_Action = 'NO CHANGE';
				    }
				} else {
				    if (prev_status.is_empty && status.a000_Action.equals("CHANGE")) {
                        if(status.a000_Action != "INSERT"){
				    	    status.a000_Action = 'NO CHANGE';
                        }
				    }
				}
				...
				pos.a006_EndDate = job.end_date.size()==0?"9999-12-31":job.end_date;
				//get position
				...
				if (domain_data != null) {
					...
					loc.a015_PhysicalAddress.a003_StartDate = fo_loc.addressStartDate;
					loc.a015_PhysicalAddress.a006_EndDate = fo_loc.addressEndDate;
				...
				pos.a051_EmployeeSubGroup.fillPicklistV2_nonUnique(domain, helper.getCurrent(job.employment_type), "employmentType", pos.a003_StartDate);
				...
				if (tmp_country == "CAN") {
					...
					pos.a052_AnnualSalary.a003_Currency.a003_Name = (domain_data == null) ? "CAD" : domain_data.description;
				...
				prev_pos.a009_Code = helper.getPrevious(job.position);
				...
				prev_pos.a054_WeeklyWorkHours = helper.getPrevious(job.standard_hours);				
				//prev job
				prev_pos.a015_Job = new Picklist();
				prev_pos.a015_Job.fillPicklistWithDomain(domain, domain.FOJobs, helper.getPrevious(job.job_code), pos.a003_StartDate);
				prev_pos.a015_Job.checkObject();
				prev_pos.a018_SupervisorEmployeeNumber = helper.getPrevious(job.manager_id);
				//get location
				prev_pos.a021_Location = new Location();
				prev_pos.a021_Location.a003_Number = helper.getPrevious(job.location);
				...
				if (domain_data != null) {
					...
					prev_pos.a021_Location.a015_PhysicalAddress.a003_StartDate = fo_loc.addressStartDate;
					prev_pos.a021_Location.a015_PhysicalAddress.a006_EndDate = fo_loc.addressEndDate;
					if (fo_loc.country.toUpperCase() == 'CANADA') {
				...
				prev_pos.a021_Location.a015_PhysicalAddress.a054_StandardTimeZone.a006_LocalName.a006_Description = helper.getPrevious(job.timezone);
				...
				prev_pos.a021_Location.a015_PhysicalAddress.a060_DaylightSavingsTimeZone.a006_LocalName.a003_Code = helper.getPrevious(job.timezone);
				...
				prev_pos.a021_Location.a018_Company.a003_Number = helper.getPrevious(job.company);
				...
				prev_pos.a021_Location.a018_Company.a009_Country.a012_ISO3CharacterCode = domain_data == null ? null : domain_data.territoryCode;
				prev_pos.a021_Location.a018_Company.a009_Country.a003_Name = domain_data == null ? null : domain_data.territoryName;
				...
				prev_pos.a021_Location.a021_CompanyTerritory.a003_Number = helper.getPrevious(job.company_territory_code);
				...
				prev_pos.a021_Location.a024_Division.a003_Number = helper.getPrevious(job.division);
				...
				prev_pos.a021_Location.a027_Region.a003_Number = helper.getPrevious(job.custom_string2);
				...
				prev_pos.a021_Location.a030_District.a003_Number = helper.getPrevious(job.custom_string3);
				...
				prev_pos.a024_CostCenter.fillPicklistWithDomain(domain, domain.FOCostCenters, helper.getPrevious(job.cost_center), pos.a003_StartDate);
				prev_pos.a024_CostCenter.checkObject();
				//get department
				prev_pos.a027_Department = new Picklist();
				prev_pos.a027_Department.fillPicklistWithDomain(domain, domain.FODepartments, helper.getPrevious(job.department), pos.a003_StartDate);
				prev_pos.a027_Department.checkObject();
				
				prev_pos.a030_PayScaleCategory = new Picklist();
				prev_pos.a030_PayScaleCategory.fillPicklistWithDomain(domain, domain.FOPayscaleTypes, helper.getPrevious(job.payScaleType), pos.a003_StartDate);
				prev_pos.a030_PayScaleCategory.checkObject();
				prev_pos.a033_PayScaleArea = new Picklist();
				prev_pos.a033_PayScaleArea.fillPicklistWithDomain(domain, domain.FOPayscaleAreas, helper.getPrevious(job.payScaleArea), pos.a003_StartDate);
				prev_pos.a033_PayScaleArea.checkObject();
				prev_pos.a036_PayScaleGroup = new Picklist();
				prev_pos.a036_PayScaleGroup.fillPicklistWithDomain(domain, domain.FOPayscaleGroups, helper.getPrevious(job.payScaleGroup), pos.a003_StartDate);
				prev_pos.a036_PayScaleGroup.checkObject();
				prev_pos.a039_PayScaleLevel = new Picklist();
				prev_pos.a039_PayScaleLevel.fillPicklistWithDomain(domain, domain.FOPayscaleLevels, helper.getPrevious(job.payScaleLevel), pos.a003_StartDate);
				prev_pos.a039_PayScaleLevel.checkObject();
				prev_pos.a042_PersonnelArea = new Picklist();
				prev_pos.a042_PersonnelArea.fillPicklistV2(domain, helper.getPrevious(job.custom_string15), "personnelArea", pos.a003_StartDate);
				prev_pos.a042_PersonnelArea.checkObject();
				prev_pos.a045_PersonnelSubArea = new Picklist();
				prev_pos.a045_PersonnelSubArea.fillPicklistV2(domain, helper.getPrevious(job.custom_string16), "personnelSubarea", pos.a003_StartDate);
				prev_pos.a045_PersonnelSubArea.checkObject();
				prev_pos.a048_EmployeeGroup = new Picklist();
				prev_pos.a048_EmployeeGroup.fillPicklistV2(domain, helper.getPrevious(job.employee_class), "EMPLOYEECLASS", pos.a003_StartDate);
				prev_pos.a048_EmployeeGroup.checkObject();
				prev_pos.a051_EmployeeSubGroup = new Picklist();
				prev_pos.a051_EmployeeSubGroup.fillPicklistV2(domain, helper.getPrevious(job.employment_type), "employmentType", pos.a003_StartDate);
				prev_pos.a051_EmployeeSubGroup.checkObject();
				prev_pos.a052_AnnualSalary = new AnnualSalary();
				prev_pos.a052_AnnualSalary.a006_Amount = helper.getPrevious(job.custom_double2);
				...
				if (tmp_country == "CAN") {
					...
					prev_pos.a052_AnnualSalary.a003_Currency.a003_Name = (domain_data == null) ? "CAD" : domain_data.description;
				...
				prev_pos.a060_WorkScheduleRule.fillPicklistWithDomain(domain, domain.FOWorkSchedules, helper.getPrevious(job.workschedule_code), pos.a003_StartDate);
			...
			for (def comp in emp.compensation_information) {
				...
				for (def recur in comp.paycompensation_recurring) {
					...
					pay.a006_EndDate = recur.end_date.size()==0? "9999-12-31":recur.end_date;
					...
					prev_pay.a009_WageCategory.fillPicklistWithDomain(domain, domain.FOPaycomponents,helper.getPrevious(recur.pay_component), pay.a003_StartDate);
					prev_pay.a009_WageCategory.checkObject();
					prev_pay.a015_Currency = new Currency();
					prev_pay.a015_Currency.a006_ISO3CharacterCurrencyCode = helper.getPrevious(recur.currency_code);
					...
					prev_pay.a012_Amount = helper.getPrevious(recur.paycompvalue);
			...
			eeOut.a078_PreviousEmployee.a021_OriginalHireDate = helper.getPrevious(emp.originalStartDate);
			eeOut.a078_PreviousEmployee.a024_AdjustedHireDate = helper.getPrevious(emp.custom_date4);
			eeOut.a078_PreviousEmployee.a025_MostRecentHireDate = helper.getPrevious(emp.start_date);
			eeOut.a078_PreviousEmployee.a027_ProbationEndDate = helper.getPrevious(emp.custom_date23);
			eeOut.a078_PreviousEmployee.a030_SeniorityDate = helper.getPrevious(emp.seniorityDate);
			eeOut.a078_PreviousEmployee.a015_PreferredLanguage = prev_lang;
			eeOut.a078_PreviousEmployee.a048_RehireIndicator = helper.getPrevious(emp.okToRehire);
			eeOut.a078_PreviousEmployee.a036_TerminationDate = helper.getPrevious(emp.end_date);
			eeOut.a078_PreviousEmployee.a033_ServiceDate = helper.getPrevious(emp.custom_date25);
			...
			tmpEE.a036_TerminationDate = helper.getCurrent(emp.end_date);
			tmpEE.a033_ServiceDate = helper.getCurrent(emp.custom_date25);
		...
		if (no_emp_info == false) {
			...
			eeOut.a036_TerminationDate = tmpEE.a036_TerminationDate;
			eeOut.a033_ServiceDate = tmpEE.a033_ServiceDate;
			//eeOut.a000_Action = tmpEE.a000_Action;
		} 
		eeOut.a000_Action = helper.getCurrent(it.person.action);
		
		if (eeOut.a000_Action == "NO CHANGE" && eeOut.a069_Event.size() > 0) {
			for (Event conv in eeOut.a069_Event) {
				
				if (conv.a000_Action == "INSERT" && conv.a009_EventCategory.a003_Code == "5" &&
					conv.a012_EventReason.a003_Code == "CONV" ) {
					eeOut.a000_Action = conv.a000_Action;
					break;
				}
			}
		}
		
		//For fields that are coming from job_information node as the canonical model is not
		//mapping the compoundEE exactly
		if (eeOut.a000_Action == "NO CHANGE" && !eeOut.a078_PreviousEmployee.is_empty) {
			eeOut.a000_Action = "CHANGE";
		} else if (eeOut.a000_Action == "CHANGE" && eeOut.a078_PreviousEmployee.is_empty) {
			eeOut.a000_Action = "NO CHANGE";
		}
		
		eeOut.a001_SourceDateTime = helper.formatSourceTime(it.person.last_modified_on);
		if (eeProcessed == "") {
			eeProcessed = eeOut.a003_EmployeeNumber;
		} else {
			eeProcessed = eeProcessed + "," + eeOut.a003_EmployeeNumber;
		}
	} // end of CompoundEE
	...
	//employees.each {
		if (e.a003_EmployeeNumber != null && e.a003_EmployeeNumber != "") {
			helper.generateXML(e);
		}
	}
	...
	if (helper.writer.toString().length() == 0) {
		...
		String eeXML = helper.writer.toString();
		String uuid = map.get("TRACKID");
		String contextXML = "<Context>";
		contextXML = contextXML + "<MessageName>CAN000_DELTA</MessageName>";
		contextXML = contextXML + "<Timestamp>" + timestamp + "</Timestamp>";
		contextXML = contextXML + "<SendingApplication>HCI</SendingApplication>";
		contextXML = contextXML + "<TrackingId>"+uuid+"</TrackingId>";
		contextXML = contextXML + "<UserId>test</UserId>";
		contextXML = contextXML + "</Context>";
		
	
		message.setBody("<Request xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance'>"+contextXML+eeXML+"</Request>");
		if(messageLog != null){
			messageLog.setStringProperty("Batch "+count+" ("+uuid+"):", eeProcessed);
		}
	}
